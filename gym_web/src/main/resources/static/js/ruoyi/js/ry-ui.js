    ...
    $.extend({
    	...
    	table: {
    		...
    		exportExcel: function(formId) {
    			$.modal.confirm("Do you confirm to export all  data (Limit 100000 Records) ?", function() {
	    			var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
			...
			exportSelectedExcel: function(formId) {
				$.modal.confirm("Do you confirm to export all  data ?", function() {
            	...
            	layer.open({
            		...
            		btn1: function(index, layero){
            			...
            			if (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx'))){
            				$.modal.msgWarning("please select “xls” or “xlsx”  format files");
            				return false;
            			}
    	...
    	modal: {
            ...
            openOptions: function (options) {
            	...
            	var _title = $.common.isEmpty(options.title) ? "System wondow" : options.title;
        ...
        operate: {
			...
			// 提交数据
			saveMap: function(url, type, dataType, data,contentType, callback) {
				var config = {
					url: url,
					type: type,
					dataType: dataType,
			...
			// 提交数据
			saveAndefresh: function(url, type, dataType, data,contentType, callback) {
				var config = {
					url: url,
					type: type,
					dataType: dataType,
            ...
            detail: function(id, width, height) {
            	...
            	var options = {
       				title: $.table._option.modalName + "Detail",
       				width: width,
       				height: height,
            ...
            // 删除信息
            remove: function(id) {
            	$.modal.confirm("Do you confirm to delete this record？", function() {
            ...
            removeAll: function() {
        		...
        		$.modal.confirm("Do you confirm to  delete the selected data?", function() {
            ...
            // 清空信息
            clean: function() {
            	$.modal.confirm("Are you sure you want to clear  all ?", function() {
            ...
            // 添加信息
            add: function(id) {
                $.modal.open("Add " + $.table._option.modalName, $.operate.addUrl(id));
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                $.modal.openTab("Add " + $.table._option.modalName, $.operate.addUrl(id));
            },
            // 添加信息 全屏
            addFull: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
                $.modal.openFull("Add " + $.table._option.modalName, url);
            },
            // 添加访问地址
            addUrl: function(id) {
            ...
            edit: function(id) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open("Edit " + $.table._option.modalName, url);
            	} else {
            	    $.modal.open("Edit " + $.table._option.modalName, $.operate.editUrl(id));
            	}
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            	$.modal.openTab("Edit " + $.table._option.modalName, $.operate.editUrl(id));
            },
            // 修改信息 全屏
            editFull: function(id) {
            	...
            	$.modal.openFull("Edit " + $.table._option.modalName, url);
            },
            // 修改访问地址
            editUrl: function(id) {
            	var url = "/404.html";
			...
			saveAndRefresh1: function(url, type, dataType, data,contentType, callback) {
				var config = {
					url: url,
					type: type,
					dataType: dataType,
            ...
            saveSuccess: function (result) {
            	if (result.code == web_status.SUCCESS) {
            		$.modal.msgReload("Saved successfully, refreshing data. Please wait ...", modal_status.SUCCESS);
                } else if (result.code == web_status.WARNING) {
                    $.modal.alertWarning(result.msg)
                }  else {
                	$.modal.alertError(result.msg);
        ...
        tree: {
        	...
        	notAllowParents: function(_tree) {
    		    ...
    		    if(nodes.length == 0){
                    $.modal.msgError("Please select a node and submit");
                    return false;
				}
    		    for (var i = 0; i < nodes.length; i++) {
    		        if (nodes[i].level == 0) {
    		            $.modal.msgError("Cannot select root node（" + nodes[i].name + "）");
    		            return false;
    		        }
    		        if (nodes[i].isParent) {
